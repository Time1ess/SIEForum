...
import Problems from 'misago/components/users/problems/root';
...
export function paths() {
  ...
  misago.get('USERS_LISTS').forEach(function(item) {
    ...
    } else if (item.component === 'problems'){
      paths.push({
        path: misago.get('USERS_LIST_URL') + item.component + '/:page/',
        component: connect(select)(Problems),
        extra: {
          name: item.name
        }
      });
